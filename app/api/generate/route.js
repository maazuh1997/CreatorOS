...
function fallback(input) {
  ...
  const focus = input.selectedAngle || 'Strategic'
  const concepts = [
  ...
  return {
    strategy: `${objective} campaign for ${audience} on ${platform}, built around five distinct creative mechanisms instead of repeated copy variations.`,
    rationale: `Each route has a different job: educate, challenge, simplify, humanize or prove. Select the mechanism that best matches the audience's current state and your brand voice.`,
    audienceInsight: `The audience needs a clear reason to care about ${topic} and an easy next step that fits ${objective.toLowerCase()}.`,
    contentPillars: ['Insight', 'Point of view', 'Practical value', 'Human story', 'Proof'],
    concepts,
    hashtags: [`#${slugTag(topic) || 'Content'}`, `#${slugTag(platform)}`, '#ContentStrategy', '#SocialMediaMarketing', '#CreatorOS']
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    const usageResponse = await fetch(new URL('/api/usage', request.url), { method: 'POST', headers: { cookie: request.headers.get('cookie') || '' } })
    const usage = await usageResponse.json()
    if (!usageResponse.ok || !usage.allowed) return NextResponse.json({ error: usage.error || 'Daily generation limit reached.' }, { status: usageResponse.status || 429 })
    let result
