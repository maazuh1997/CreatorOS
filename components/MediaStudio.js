...
export default function MediaStudio({ concept, platform }) {
  ...
  function selectAsset(item) { const asset = { ...item, type, mediaUrl: mediaUrl(item), selectedFor: platform, selectedAt: new Date().toISOString(), concept }; setSaved(item.id); setSelected(item); localStorage.setItem('creatoros_selected_media', JSON.stringify(asset)); sessionStorage.setItem('creatoros_selected_media', JSON.stringify(asset)); setTimeout(() => setSaved(null), 1800) }
